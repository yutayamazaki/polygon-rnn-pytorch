...
if __name__ == '__main__':
    ...
    import random
    utils.seed_everything(random.randint(0, 1000))
    ...
    for img_path in images[:10]:
        ...
        orig_image: Image.Image = transforms.ToPILImage()(x[0]).resize((28, 28))
        ...
        orig_image2: Image.Image = transforms.ToPILImage()(x[0]).resize((28, 28))
        draw = ImageDraw.Draw(orig_image2)
        polygon_anno: List[Tuple[int, int]] = [(index2point(int(idx))[0], index2point(int(idx))[1]) for idx in gt[0]]
        draw.line(xy=sort_polygon(polygon_anno), fill=255)
        ax3.imshow(orig_image2)
